...
create_apkovl_overlay() {
    ...
    rm -rf "$overlay_dir"
    mkdir -p "$overlay_dir"/{etc/profile.d,etc/network,etc/apk,etc/local.d,root,opt}
...
EOF
    ...
    # Network config with improved udhcpc options
...
EOF
    ...
    # Local startup script with improved networking
...
integrate_overlay_into_iso() {
    ...
    # Modify boot configuration to auto-load overlay
    ...
    if [[ -f "$syslinux_cfg" ]]; then
        ...
        # Add apkovl parameter to kernel boot line
        sed -i 's/quiet/quiet apkovl=\/cyberxp.apkovl.tar.gz/' "$syslinux_cfg"
        
        log_success "Boot configuration updated"
    else
        log_warn "syslinux.cfg not found, overlay must be loaded manually"
    fi
}
